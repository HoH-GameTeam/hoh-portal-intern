...
import { SettingsContextProps, LAYOUT_OPTION, ThemeModeValue } from './types';
...
export function SettingsProvider({ children }: SettingsProviderProps) {
  ...
  useEffect(() => {
    setSettings({ ...settings, themeHeader: true, themeBottomBar: true });
  }, []);
  ...
  const onChangeLayout = useCallback(
    (layoutOption: LAYOUT_OPTION) => {
      const themeLayout = layoutOption;
      setSettings({ ...settings, themeLayout });
    },
    [setSettings, settings],
  );
  ...
  const onToggleHeader = useCallback(
    (toggle: boolean) => {
      const themeHeader = toggle;
      setSettings({ ...settings, themeHeader });
    },
    [setSettings, settings],
  );
  ...
  const onToggleBottomBar = useCallback(
    (toggle: boolean) => {
      const themeBottomBar = toggle;
      setSettings({ ...settings, themeBottomBar });
    },
    [setSettings, settings],
  );
